...
**Table S1 -** Actual and predicted data for each diameter. Here we only present the first 6 rows of data for brevity. Note units and proper notation are missing due to restrictions for data.frame objects. From left to right: particle diameter (nm), predicted concentration (dN/dlogD~p~), predicted number concentration (n cm^-3^), actual measurment concentration (dN/dlogD~p~), actual number concentration (n cm^-3^), residual particle concentration (dN/dlogD~p~), residual number concentration (n cm^-3^), and ratio of actual to predicted concentration for each bin.

```{r}

knitr::kable(result[[3]][1:6, ], format="html")
```
...
knitr::kable(result[[4]][1:6, ], format="html")

```

**Table S3 -** Lognormal parameters, peak identification diameters, and statistics. From left to right: mode label, total number concentration (n cm^-3^), geometric mean diameter (nm), max concentration (dN/dlogD~p~) of identified peak, lower diameter limit (nm), mode diameter (nm), upper diameter limit (nm), width in bin resolution of file, Bayesian Information Criterion, residual sum of squares, total sum of squares, coefficient of correlation, and Student's t-test significance values for each fitted mode.

```{r}

knitr::kable(result[[5]][1:6, ], format="html")
```
...
plots$`2022-03-23 15:30:00`
plots$`2022-03-23 15:55:00`

```

Now for the hour of data, 10 of the 12 fittings (83% were successful) (i.e. explained 90% of concentration variance and had a NRMSE of 0.1).
